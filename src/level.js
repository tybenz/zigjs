                ...
                for ( j = 0, l2 = row.length; j < l2; j++ ) {
                    ...
                    for ( k = 0, l3 = ents.length; k < l3; k++ ) {
                        ...
                        var spriteMatch = entityStr.match( /\{([^\{\}]*)\}/ ),
                            ...
                            sizeRegex = /\((.*)x(.*)\)/,
                            sizeMatch = entityStr.match( sizeRegex ),
                            sizeX = sizeMatch ? sizeMatch[1] : null,
                            sizeY = sizeMatch ? sizeMatch[2] : null;

                        entityStr = entityStr.replace( /\{[^\{\}]*\}/, '' ).replace( sizeRegex, '' );
                        if ( entityStr != 'blank' ) {
                            ...
                            if ( sizeX && sizeY ) {
                                entity.width = sizeX * tilesize;
                                entity.height = sizeY * tilesize;
                            }
